...
def bc_stat():
    return _bc_request("GET", "/service/stat")

def bc_maintenance(on):
    return _bc_request("POST", "/service/maintenance", json={"on": bool(on)})
...
def save_event_and_job(event):
    """Атомарно: событие + задача. True если новое, False если дубль."""
    with conn() as c, c.cursor() as cur:
